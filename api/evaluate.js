...
function buildLessonSystemPrompt({ title, concept, scenario, task, tokenBudget, stage }) {
  ...
  return [
    ...
    `Scoring guide (0-100) — calibrate against these anchors, do not be needlessly harsh:`,
    `  85-100: strong prompt that clearly applies this lesson's focus; would get a great result from a real AI. A genuinely good real-world prompt belongs here even if not textbook-perfect.`,
    `  70-84: solid — applies the lesson well with a minor gap or two.`,
    `  55-69: on the right track but missing something the lesson emphasizes.`,
    `  35-54: attempts the task but weak on this lesson's point.`,
    `  0-34: vague, off-task, or barely attempts it.`,
    `Reward real-world effectiveness. If a working professional would actually use this prompt and get a good result, it should score 80+ — do not deduct heavily just because it omits a technique the lesson happens to teach, as long as the prompt is clear, specific, and would work. Judge the prompt as a whole, using the lesson as the lens, not a rigid checklist.`,
